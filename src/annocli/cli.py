...
def main():
    parser = argparse.ArgumentParser(description="annocli: command-line tool to query and download genome annotations")
    ...
    download_parser.add_argument("--add_asm", action="store_true", help="Download also assemblies")
    download_parser.add_argument(
        "--fix_alias",
        action="store_true",
    ...
    ##### Alias command
    alias_parser = subparsers.add_parser("alias", help="Match alias between annotation and assembly")
    ...
    ##### Describe command
    describe_parser = subparsers.add_parser("describe", help="Get information about features and biotypes availble")
    ...
    if args.command == "download":
        ...
        if args.mode in ["dw", "links"]:
            ...
            for result in annotations_json.get("results", []):
                ...
                annotation_folder = os.path.join(args.output, "_".join([organism_name, taxid]), assembly_accession)
                            ...
                            try:
                                ...
                                if args.fix_alias and source_filepath:
                                    ...
                                    if source_filepath.endswith(".gz"):
                                        extention = ".".join(source_filepath.split(".")[-2:])

                                    else:
                                        extention = source_filepath.split(".")[-1]

                                    alias_fixed_filepath = source_filepath.replace(extention, f"aliasMatch.{extention}")
