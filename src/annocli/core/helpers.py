...
def rewrite_gff_seqids_from_assembly(
    ...
    asm_cmd = (
        f"gzip -cd {_sh_quote(asm_fna_gz)} | "
    ...
    region_cmd = (
        f"gzip -cd {_sh_quote(ann_gff_gz)} | "
    ...
    # 3) Rewrite GFF
    with gzip.open(ann_gff_gz, "rt", encoding="utf-8", errors="replace") as ann_in, \
         gzip.open(out_gff_gz, "wt", encoding="utf-8", errors="replace") as ann_out:
